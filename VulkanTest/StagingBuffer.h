...
public:
	...
	StagingBuffer(VkDevice device, VkPhysicalDeviceMemoryProperties memprops, VkDeviceSize size, VkBufferUsageFlagBits usage = VK_BUFFER_USAGE_TRANSFER_SRC_BIT) 
		: Buffer(device, memprops) {
		this->size = size;

		this->initBuffer(usage, size, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT);
	};
